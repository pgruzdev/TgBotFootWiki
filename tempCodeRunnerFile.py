...
def load_facts(filename):
    ...
    try:
        with open(filename, "r", encoding="utf-8") as file:
            for line in file:
                # Убедимся, что строка не пустая и содержит разделитель
                if "|" in line:
...
def site_command(message):
    # Заменили webbrowser на отправку ссылки кнопкой, чтобы это работало у всех пользователей
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('Открыть профиль ВК', url='https://vk.com'))
    bot.send_message(message.chat.id, "Кликни по кнопке ниже, чтобы перейти на сайт:", reply_markup=markup)

@bot.message_handler(content_types=['photo'])
def get_photo(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('Перейти в официальный тг канал', url='https://t.me'))
    bot.reply_to(message, 'Нормик фотка', reply_markup=markup)


# ===============================
# ОБРАБОТЧИК ТЕКСТА (ГЛАВНАЯ ЛОГИКА)
# ===============================

# Важно: этот обработчик стоит в самом низу и обрабатывает текстовые сообщения
@bot.message_handler(content_types=['text'])
def handle(message):
    chat_id = message.chat.id
    text = message.text

    # --- Отдельная проверка на команду ID (теперь она не блокирует остальной код) ---
    if text.lower() == 'id':
    ...
    if text == "⬅ Назад":
        ...
        # Если был выбран клуб → возвращаем к выбору клуба
    ...
    if text == "🇪🇸 Barcelona":
        ...
        bot.send_message(chat_id, "Visca el Barça i Visca Catalunya!\nЧто хочешь узнать?", reply_markup=fact_menu())
        return
...
if __name__ == "__main__":
    print("Бот успешно запущен на локальных файлах...")
